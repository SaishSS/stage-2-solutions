...
public class Source {
    public static void main(String[] args) {
        ...
        if (input == 1) {
            ...
            customer = CS.searchCustomer(id);
            actualresult = customer.toString();
            System.out.println(actualresult);
        }
        if (input == 2) {
            Toy toy = AS.searchToy(id);
...
class Customer {
    ...
    private String customerName, email, password, address;
...
class Toy {
    int toyId, minAge, maxAge, rentalAmt, refundableDeposit, quantity;
    String toyName, toyType, toString;
    double price;
